...
async function createTestEnvironment () {
  ...
  while (!defaultLocale) {
    try {
      defaultLocale = await environment
        .getLocales()
...
// Disable for now as we need to rethink integration test strategy
describe.skip('Integration Test @integration', () => {
  ...
  describe('init command', () => {
    it('should create the migration content model', async () => {
      spawnSync(NODE_CMD, [MIGRATION_CMD, 'init', ...TOKEN_SPACE_ENV_OPTIONS])
  ...
  describe('create command', () => {
    it('should create a template file in migrations folder', async () => {
      ...
      const migrationScripts = fs.readdirSync(
  ...
  describe('up command', () => {
    ...
    it('-c on same content type without new script should not change last run', async () => {
      ...
      const previousMigrationsLastRun =
        previousMigration.fields.state[defaultLocale].migrations[0].timestamp
      ...
      const currentMigrationLastRun =
        currentMigration.fields.state[defaultLocale].migrations[0].timestamp
  ...
  describe('bootstrap command', () => {
    ...
    it('-c option with no response only creates script for content type', async () => {
      ...
      await environment
        .createContentTypeWithId(contentType, contentTypeOptions)
    ...
    it('-a option with no response creates scripts for all content types', async () => {
      ...
      await environment
        .createContentTypeWithId('alpaca', contentTypeOptions)
        .then((createdContentType) => createdContentType.publish())
    ...
    it('-a option with yes overrides the state for all content type', async () => {
      ...
      migrations.items.forEach((migration) => {
        ...
        const migrationScriptsFolder = path.join(
          MIGRATIONS_FOLDER,
          migration.sys.id
        )
        const migrationScripts = fs.readdirSync(migrationScriptsFolder)
